...
JUNK_TITLES = [
    ...
    "news18", "ndtv.com", "timesofindia", "hindustantimes",
]
JUNK_DOMAINS = [
...
def fetch_all_news():
    ...
    # Pure politics, governance, world affairs
    # No sports/finance keywords — avoids category bleed
    try:
        r1 = client.search(
            query=(
                "India breaking news today 2026 "
                "Modi government parliament policy minister "
                "world international diplomacy foreign"
            ),
            search_depth="advanced",
            max_results=15,
            days=1
        )
        for a in r1.get("results", []):
            if is_junk(a):
                continue
            t = (
                (a.get("title",   "") or "") + " " +
    ...
    # Markets, cricket, Bihar — all in one call
    try:
        r2 = client.search(
            query=(
                "India breaking news today 2026 "
        ...
        for a in r2.get("results", []):
            ...
            # Priority: Regional > Sports > Finance > National
            if "rosera" in t:
