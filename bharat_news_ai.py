...
# ── Keywords — strict, non-overlapping ───────────────────────────
INTL_EXACT = [
    ...
    "elon musk", "tariff war", "trade war"
]
INTL_WORDS = [
...
SPORTS_EXACT = [
    ...
    "pro kabaddi", "indian super league"
]
SPORTS_WORDS = [
    "cricket wicket", "batting innings", "bowling figures",
    "run chase", "match score", "football goal",
    "tennis grand slam", "badminton championship",
    "hockey world cup", "athletics gold medal"
]
FINANCE_EXACT = [
...
def classify_general(article):
    """
    Only for Call 1 and Call 2 results.
    Never called for regional (Call 3).
    """
    t = (
...
def classify_regional(article):
    """
    Only for Call 3 results.
    No sports/finance keywords — pure location match.
    """
    t_url = (
        (article.get("title",   "") or "") + " " +
        (article.get("content", "") or "") + " " +
    ...
    return "bihar"  # default for Call 3
...
def fetch_all_news():
    ...
    for a in sorted(
        ...
        cat = classify_general(a)
        # Force to national or international only
        if cat == "international":
            cats["international"].append(a)
        else:
            cats["national"].append(a)

    # Call 2 → finance + sports ONLY
    for a in sorted(
        ...
        cat = classify_general(a)
        # Force to finance or sports only
        if cat == "sports":
            cats["sports"].append(a)
        else:
            cats["finance"].append(a)

    # Call 3 → regional ONLY — use classify_regional, never classify_general
    for a in sorted(
        [x for x in r3 if not is_junk_regional(x)],
        key=lambda x: x.get("published_date","") or "",
        reverse=True
    ):
        cat = classify_regional(a)
        cats[cat].append(a)
...
def build_words(articles_json, label):
    ...
    prompt = (
        ...
        f"Return ONLY a valid JSON array. No markdown. Nothing else.\n"
        f'[{{"word":"example","meaning":"clear definition",'
        f'"usage":"sentence using the word"}}]\n\n'
        f"Text:\n{text}"
    )
    ...
    # Try greedy regex — finds largest array in response
    try:
        m = re.search(r'\[.*\]', clean, re.DOTALL)  # greedy — not \[.*?\]
        if m:
...
with tab5:
    ...
    if bihar:
        ...
        st.caption("No Bihar news found right now.")
    ...
    if sams:
        ...
        st.caption("No Samastipur news found right now.")

    st.divider()

    # Rosera — always shown as separate section
    ...
    if rosera:
        ...
        st.caption("No Rosera news found right now.")
